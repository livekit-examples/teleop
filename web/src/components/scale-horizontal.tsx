...
export function ScaleHorizontal({
  ...
  return (
    <div
      className={cn(
        "text-foreground grid grid-rows-[auto_auto] gap-1 font-mono select-none",
        className,
      )}
    >
      {/* Horizontal value */}
          ...
          {ticks.map((deg) => {
            ...
            return (
              <div
                ...
                style={{ left: x }}
              >
                <div className={cn(`bg-card/25 w-0.5`, isZero ? "h-3" : "h-2")} />
                <span className="text-foreground/50 shrink-0 font-mono text-xs leading-6 whitespace-pre">
                  {deg >= 0 ? ` ${deg}°` : `${deg}°`}
                </span>
              </div>
            );
